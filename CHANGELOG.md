...
- YouTube video thumbnails and playback (Home "Notre vision" section, WeekPaper episodes) were blocked on the deployed site by the CSP, which lacked `img.youtube.com`/`i.ytimg.com` in `img-src` and `www.youtube.com` in `frame-src`
